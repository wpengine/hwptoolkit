...
if ! docker ps | grep -q hwp-previews-wordpress-1; then
    echo "❌ Error: Container hwp-previews-wordpress-1 is not running"
    exit 1
fi
...
check_extension() {
    local extension=$1
    docker exec hwp-previews-wordpress-1 php -m | grep -q "$extension"
}
...
if check_extension "pcov"; then
    ...
    if docker exec hwp-previews-wordpress-1 pecl list | grep -q "pcov"; then
        echo "PCOV is installed via PECL but not enabled. Enabling..."
        docker exec hwp-previews-wordpress-1 bash -c "
...
if docker exec hwp-previews-wordpress-1 wp core is-installed --allow-root 2>/dev/null; then
...
if docker exec hwp-previews-wordpress-1 wp plugin is-active hwp-previews --allow-root 2>/dev/null; then
    echo "✓ Plugin is active"
else
    echo "Activating plugin..."
    docker exec hwp-previews-wordpress-1 wp plugin activate hwp-previews --allow-root
fi

# Verify coverage driver is working
echo "Verifying code coverage setup..."
if docker exec hwp-previews-wordpress-1 php -r "
...
echo "  docker exec -e COVERAGE=1 -e SUITES=wpunit -w /var/www/html/wp-content/plugins/hwp-previews hwp-previews-wordpress-1 bin/run-codeception.sh"
echo ""
echo "Or without coverage:"
echo "  docker exec -e SUITES=wpunit -w /var/www/html/wp-content/plugins/hwp-previews hwp-previews-wordpress-1 bin/run-codeception.sh"
