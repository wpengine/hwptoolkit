    ...
    public static function register(): void {
        register_graphql_mutation('updateWebhook', [
            'inputFields' => [
                'id' => [
                    'type' => 'ID',
                    'description' => __('The ID of the webhook to update', 'wp-graphql-headless-webhooks'),
                ],
                'title' => [
                    'type' => 'String',
                    'description' => __('The new title of the webhook', 'wp-graphql-headless-webhooks'),
                ],
                'content' => [
                    'type' => 'String',
                    'description' => __('The new content/description of the webhook', 'wp-graphql-headless-webhooks'),
                ],
                'eventTrigger' => [
                    'type' => 'String',
                    'description' => __('The new event hook name that triggers this webhook', 'wp-graphql-headless-webhooks'),
                ],
                'enabled' => [
                    'type' => 'Boolean',
            ...
            'outputFields' => [
                'webhook' => [
                    'type' => 'Webhook',
                    'description' => __('The updated webhook', 'wp-graphql-headless-webhooks'),
            ...
            'mutateAndGetPayload' => function ($input, $context, $info) {
                if (!current_user_can('manage_options')) {
                    throw new UserError(__('You do not have permission to update webhooks.', 'wp-graphql-headless-webhooks'));
                }

                if (empty($input['id'])) {
                    throw new UserError(__('The ID of the webhook to update is required.', 'wp-graphql-headless-webhooks'));
                }

                $post_id = is_numeric($input['id']) ? (int) $input['id'] : 0;

                if ($post_id <= 0) {
                    throw new UserError(__('Invalid webhook ID.', 'wp-graphql-headless-webhooks'));
                }

                $post = get_post($post_id);
                if (!$post || $post->post_type !== 'graphql_webhook') {
                    throw new UserError(__('Webhook not found.', 'wp-graphql-headless-webhooks'));
                }
                ...
                if (is_wp_error($updated_post_id)) {
                    throw new UserError(__('Failed to update webhook.', 'wp-graphql-headless-webhooks'));
                }
