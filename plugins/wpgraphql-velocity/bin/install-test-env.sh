...
configure_wordpress() {
	...
	SITE_TITLE=${WORDPRESS_SITE_TITLE:-"WPGraphQL Velocity Tests"}
...
setup_plugin() {
	if [ "${SKIP_WP_SETUP}" = "true" ]; then
		echo -e "$(warning_message "Skipping wpgraphql-velocity installation...")"
		return 0
	fi

	# Add this repo as a plugin to the repo
	if [ ! -d $WORDPRESS_ROOT_DIR/wp-content/plugins/wpgraphql-velocity ]; then
