...
class WebhooksAdmin {
	...
	public function add_admin_menu(): void {
		...
		add_menu_page(
			__( 'Webhooks', 'wp-graphql-webhooks' ),
			__( 'Webhooks', 'wp-graphql-webhooks' ),
			'manage_options',
		...
		add_submenu_page(
			'graphiql-ide',
			__( 'GraphQL Webhooks', 'wp-graphql-webhooks' ),
			__( 'Webhooks', 'wp-graphql-webhooks' ),
			'manage_options',
			self::ADMIN_PAGE_SLUG,
			[ $this, 'render_admin_page' ]
	...
	public function enqueue_assets( string $hook ): void {
		...
		wp_localize_script(
			...
			[ 
				...
				'confirmDelete' => __( 'Are you sure you want to delete this webhook?', 'wp-graphql-webhooks' ),
	...
	public function ajax_test_webhook(): void {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wp_rest' ) ) {
			wp_send_json_error( [ 
				'message' => __( 'Invalid security token.', 'wp-graphql-webhooks' ),
		...
		if ( ! $webhook_id ) {
			wp_send_json_error( [ 
				'message' => __( 'Invalid webhook ID.', 'wp-graphql-webhooks' ),
		...
		if ( ! $webhook ) {
			wp_send_json_error( [ 
				'message' => __( 'Webhook not found.', 'wp-graphql-webhooks' ),
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error( [ 
				'message' => sprintf(
					__( 'Failed to send test webhook: %s', 'wp-graphql-webhooks' ),
		...
		$response_data = [ 
			'success' => $is_success,
			'message' => $is_success
				? sprintf( __( 'Test webhook sent successfully to %s', 'wp-graphql-webhooks' ), $webhook->url )
				: sprintf( __( 'Webhook returned HTTP %d', 'wp-graphql-webhooks' ), $response_code ),
