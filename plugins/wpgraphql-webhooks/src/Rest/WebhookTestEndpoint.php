...
class WebhookTestEndpoint {
	...
	public function test_webhook( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( ! $webhook ) {
			return new WP_Error(
				'webhook_not_found',
				__( 'Webhook not found.', 'graphql-webhooks' ),
				[ 'status' => 404 ]
			);
		}
		...
		try {
			...
			return new WP_REST_Response(
				[
					'success' => true,
					'message' => __( 'Test webhook dispatched successfully.', 'graphql-webhooks' ),
					'details' => [
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to dispatch test webhook: %s', 'graphql-webhooks' ),
					$e->getMessage()
				),
				[ 'status' => 500 ]
			);
