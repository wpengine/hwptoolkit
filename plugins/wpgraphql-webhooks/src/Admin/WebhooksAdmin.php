...
class WebhooksAdmin {
	...
	public function add_admin_menu(): void {
		...
		add_submenu_page(
			'graphiql-ide',
			__( 'GraphQL Webhooks', 'graphql-webhooks' ),
			__( 'Webhooks', 'graphql-webhooks' ),
			'manage_options',
			self::ADMIN_PAGE_SLUG,
			[ $this, 'render_admin_page' ]
	...
	public function enqueue_assets( string $hook ): void {
		...
		wp_enqueue_style(
			...
			WPGRAPHQL_WEBHOOKS_PLUGIN_URL . 'assets/css/admin.css',
			[],
		...
		wp_enqueue_script(
			'graphql-webhooks-admin',
			WPGRAPHQL_WEBHOOKS_PLUGIN_URL . 'assets/js/admin.js',
			[ 'jquery' ],
			WPGRAPHQL_WEBHOOKS_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			[ 
				...
				'confirmDelete' => __( 'Are you sure you want to delete this webhook?', 'graphql-webhooks' ),
	...
	private function verify_admin_permission(): bool {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( __( 'You do not have sufficient permissions to access this page.', 'graphql-webhooks' ) );
			return false;
		}
		return true;
	}
	...
	private function verify_nonce( string $nonce_name, string $action ): bool {
		if ( ! isset( $_REQUEST[ $nonce_name ] ) || ! wp_verify_nonce( wp_unslash( $_REQUEST[ $nonce_name ] ), $action ) ) {
			wp_die( __( 'Security check failed.', 'graphql-webhooks' ) );
			return false;
		}
		return true;
	}
	...
	public function handle_webhook_save() {
		if ( ! $this->verify_admin_permission() || ! $this->verify_nonce( 'webhook_nonce', 'webhook_save' ) ) {
			wp_die( __( 'Unauthorized', 'graphql-webhooks' ) );
		}
	...
	private function render_form_page( $action ) {
		$webhook = null;
		$form_title = 'add' === $action ? __( 'Add New Webhook', 'graphql-webhooks' ) : __( 'Edit Webhook', 'graphql-webhooks' );
		$submit_text = 'add' === $action ? __( 'Add Webhook', 'graphql-webhooks' ) : __( 'Update Webhook', 'graphql-webhooks' );
		...
		if ( 'edit' === $action ) {
			...
			if ( ! $webhook ) {
				wp_die( __( 'Webhook not found.', 'graphql-webhooks' ) );
			}
	...
	public function ajax_test_webhook(): void {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'wp_rest' ) ) {
			wp_send_json_error( [ 
				'message' => __( 'Invalid security token.', 'graphql-webhooks' ),
		...
		if ( ! $this->verify_admin_permission() ) {
			wp_send_json_error( [ 
				'message' => __( 'You do not have permission to test webhooks.', 'graphql-webhooks' ),
		...
		if ( ! $webhook_id ) {
			wp_send_json_error( [ 
				'message' => __( 'Invalid webhook ID.', 'graphql-webhooks' ),
		...
		if ( ! $webhook ) {
			wp_send_json_error( [ 
				'message' => __( 'Webhook not found.', 'graphql-webhooks' ),
		...
		$args = [ 
			...
			'headers' => array_merge(
				[ 
					...
					'User-Agent' => 'WPGraphQL-Webhooks/' . WPGRAPHQL_WEBHOOKS_VERSION,
				],
				$webhook->headers
			),
		...
		if ( is_wp_error( $response ) ) {
			wp_send_json_error( [ 
				'message' => sprintf(
					__( 'Failed to send test webhook: %s', 'graphql-webhooks' ),
		...
		$response_data = [ 
			'success' => $is_success,
			'message' => $is_success
				? sprintf( __( 'Test webhook sent successfully to %s', 'graphql-webhooks' ), $webhook->url )
				: sprintf( __( 'Webhook returned HTTP %d', 'graphql-webhooks' ), $response_code ),
