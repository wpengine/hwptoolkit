...
if ( ! defined( 'WPGRAPHQL_WEBHOOKS_TEXT_DOMAIN' ) ) {
	define( 'WPGRAPHQL_WEBHOOKS_TEXT_DOMAIN', 'wpgraphql-webhooks' );
}
...
if ( file_exists( __DIR__ . '/activation.php' ) ) {
	...
	register_activation_hook( __FILE__, 'wpgraphql_webhooks_activation_callback' );
}

// Run this function when the plugin is deactivated.
if ( file_exists( __DIR__ . '/deactivation.php' ) ) {
	require_once __DIR__ . '/deactivation.php';
	register_deactivation_hook( __FILE__, 'wpgraphql_webhooks_deactivation_callback' );
}

/**
 * Define plugin constants.
 */
function wpgraphql_webhooks_constants(): void {
	// Plugin version.
 ...
 * @return array<string>
 */
function wpgraphql_webhooks_dependencies_not_ready(): array {
	$deps = [];
 ...
 * Initializes plugin.
 */
function wpgraphql_webhooks_init(): void {
	wpgraphql_webhooks_constants();
	$not_ready = wpgraphql_webhooks_dependencies_not_ready();

	if ( $not_ready === [] && defined( 'WPGRAPHQL_WEBHOOKS_PLUGIN_DIR' ) ) {
		// Load text domain at the init hook
		add_action( 'init', 'WPGraphQL\Webhooks\wpgraphql_webhooks_load_textdomain' );
 ...
 */
function wpgraphql_webhooks_load_textdomain(): void {
...
add_action( 'init', 'WPGraphQL\Webhooks\wpgraphql_webhooks_load_textdomain', 1 );

/** @psalm-suppress HookNotFound */
add_action( 'plugins_loaded', 'WPGraphQL\Webhooks\wpgraphql_webhooks_init' );