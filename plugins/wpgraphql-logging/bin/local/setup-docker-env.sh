...
if ! docker ps | grep -q wpgraphql-logging-wordpress-1; then
    echo "❌ Error: Container wpgraphql-logging-wordpress-1 is not running"
    exit 1
fi
...
check_extension() {
    local extension=$1
    docker exec wpgraphql-logging-wordpress-1 php -m | grep -q "$extension"
}
...
if check_extension "pcov"; then
    ...
    if docker exec wpgraphql-logging-wordpress-1 pecl list | grep -q "pcov"; then
        echo "PCOV is installed via PECL but not enabled. Enabling..."
        docker exec wpgraphql-logging-wordpress-1 bash -c "
...
if docker exec wpgraphql-logging-wordpress-1 wp core is-installed --allow-root 2>/dev/null; then
...
if docker exec wpgraphql-logging-wordpress-1 php -r "
