...
export default function MiniCart({ isVisible = false, onClose }: MiniCartVisualProps) {
	const { cart, updateCartItemQuantity, removeItem, cartLoading, clearingCart, refreshCart, cartItemCount, clearCart } =
		useCart();
							...
							<div className="flex-1 overflow-y-auto p-4">
								<ul className="space-y-4">
									{cart.contents.nodes.map((item: MiniCartItem) => {
										...
										const imageUrl =
											variation?.image?.sourceUrl || item.product?.node?.image?.sourceUrl || "/placeholder.jpg";
										const imageAlt =
													...
													{variation && (
														<p className="text-sm text-gray-500">
															{variation.attributes?.nodes
																...
																?.map((attr) => {
																	const label = attr.name?.replace(/^pa_/, '').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) || '';
				...
				{isLoading && (
					<div className="absolute inset-0 bg-white bg-opacity-75 flex items-center justify-center z-10">
						<div className="flex items-center space-x-2">
							...
							<span className="text-sm text-gray-600">{clearingCart ? "Clearing cart..." : "Updating cart..."}</span>
						</div>
					</div>
				)}
			</div>
