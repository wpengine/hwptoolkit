...
export default function Cart() {
    const { cart, cartItems, loading, refreshCart } = useCart();
    ...
    if (!cart || cart.isEmpty) {
        return (
            <div className="text-center py-16">
                ...
                <Link href="/shop" className="inline-block bg-blue-600 text-white px-6 py-3 rounded-md hover:bg-blue-700 transition-colors font-medium">
                    Continue Shopping
                </Link>
            </div>
        );
                        ...
                        <div className="p-6">
                            <div className="space-y-6">
                                {cartItems.map((item) => {
                                    ...
                                    return (
                                        <div key={item.key} className={`flex items-start space-x-4 pb-6 border-b border-gray-200 last:border-b-0 last:pb-0 ${isUpdating ? 'opacity-60' : ''}`}>
                                            {/* Product Image */}
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <Link href={`/product/${product.slug}`} className="text-lg font-medium text-gray-900 hover:text-blue-600 transition-colors">
                                                    {product.name}
                                                </Link>
                                                {variation && (
                                                    <p className="text-sm text-gray-500 mt-1">{variation.name}</p>
                                                )}
